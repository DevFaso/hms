...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  criticalStripCards = computed<StatCard[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        icon: 'assignment',
        color: cs.pendingOrderReviewCount > 0 ? '#0e7c6b' : '#64748b',
        bgColor: cs.pendingOrderReviewCount > 0 ? '#ccebe4' : '#f1f5f9',
        route: '/lab',
      },
      {
        key: 'safety_alerts',
  ...
  statCards = computed<StatCard[]>(() => {
    ...
    const base: StatCard[] = [
      ...
      },
      {
        ...
        color: this.criticalAlerts().length > 0 ? '#dc2626' : '#0e7c6b',
        bgColor: this.criticalAlerts().length > 0 ? '#fee2e2' : '#e6f4f1',
      },
      {
        key: 'inbox',
  ...
  getAgingBucketColor(index: number): string {
    const colors = ['#059669', '#0e7c6b', '#d97706', '#ea580c', '#dc2626'];
