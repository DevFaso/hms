 ...
 * and the real inbound services — only repositories and EMPI are mocked —
 * because the thing under test is the bytes that leave the process, not an
 * internal enum. The MSA segment is compared whole: it carries the
...
class AdtCrossTenantAckTest {
    ...
    void setUp() {
        ...
        MllpInboundMergeServiceImpl mergeService = new MllpInboundMergeServiceImpl(
            empiService, registrationRepository);
