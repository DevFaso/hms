...
@TestPropertySource(properties = {
    ...
    // No datasource override: PR #287 pinned this class to its own H2 database
    // so a sibling's create-drop shutdown could not drop the platform schema
    // under the mllp-worker thread. The test profile now gives EVERY context
    // its own database (spring.datasource.url in application-test.yml).
})
// Close the context as soon as this class is done: its MLLP listener and
// worker threads have no later user, so the heap goes back at once.
