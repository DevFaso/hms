...
class MllpInboundLabServiceImplTest {
    ...
    // ── a preliminary/final pair is ONE observation ──────────────────────

    @Test
    @DisplayName("The final observation revises the preliminary row instead of stacking a second one")
    void finalObservationSupersedesThePreliminaryRow() {
        when(specimenRepository.findByAccessionNumber("ACC-1")).thenReturn(Optional.of(specimen));
        when(labResultRepository.save(any(LabResult.class))).thenAnswer(inv -> inv.getArgument(0));

        // The preliminary landed first and is the unreleased row for this analyte.
        LabResult preliminary = LabResult.builder().labOrder(labOrder).build();
        preliminary.setId(UUID.randomUUID());
        preliminary.setTestCode("GLU");
        preliminary.setResultValue("5.4");
        preliminary.setResultDate(LocalDateTime.of(2026, 4, 29, 8, 0));
        when(labResultRepository.findUnreleasedAnalyzerRows(labOrder.getId(), "GLU", "APP", "FAC"))
            .thenReturn(List.of(preliminary));

        MllpInboundOutcome outcome = service.processOruR01(
            List.of(observation("ACC-1", "5.9", "1", "GLU", "H", "F")),
            hospital, "APP", "FAC", null, "MSH|...\r");

        assertThat(outcome).isEqualTo(MllpInboundOutcome.ACCEPTED);
        ArgumentCaptor<LabResult> captor = ArgumentCaptor.forClass(LabResult.class);
        verify(labResultRepository).save(captor.capture());
        // The SAME row, carrying the final value — not a second one beside it.
        assertThat(captor.getValue()).isSameAs(preliminary);
        assertThat(captor.getValue().getId()).isEqualTo(preliminary.getId());
        assertThat(captor.getValue().getResultValue()).isEqualTo("5.9");
        assertThat(captor.getValue().getAbnormalFlag()).isEqualTo(AbnormalFlag.ABNORMAL_HIGH);
        assertThat(labOrder.getStatus()).isEqualTo(LabOrderStatus.RESULTED);
    }

    @Test
    @DisplayName("A stale retransmission of the superseded preliminary is ignored, not stacked")
    void staleObservationIsIgnored() {
        when(specimenRepository.findByAccessionNumber("ACC-1")).thenReturn(Optional.of(specimen));

        LabResult stored = LabResult.builder().labOrder(labOrder).build();
        stored.setId(UUID.randomUUID());
        stored.setTestCode("GLU");
        stored.setResultValue("5.9");
        stored.setResultDate(LocalDateTime.of(2026, 4, 29, 9, 0));
        when(labResultRepository.findUnreleasedAnalyzerRows(labOrder.getId(), "GLU", "APP", "FAC"))
            .thenReturn(List.of(stored));

        // The observation is an hour older than what we hold: the preliminary
        // arriving again after its final.
        MllpInboundOutcome outcome = service.processOruR01(
            List.of(observation("ACC-1", "5.4", "1", "GLU", "N", "P")),
            hospital, "APP", "FAC", null, "MSH|...\r");
        ...
        assertThat(stored.getResultValue()).isEqualTo("5.9");
    }

    @Test
    @DisplayName("Another analyzer's row for the same test code is never overwritten")
    void aDifferentSenderNeverSupersedes() {
        when(specimenRepository.findByAccessionNumber("ACC-1")).thenReturn(Optional.of(specimen));
        when(labResultRepository.save(any(LabResult.class))).thenAnswer(inv -> inv.getArgument(0));
        // The finder is scoped to the sending system, so the other analyzer's
        // row is not a candidate: this lookup returns nothing.
        when(labResultRepository.findUnreleasedAnalyzerRows(labOrder.getId(), "GLU", "OTHER", "LAB_B"))
            .thenReturn(List.of());

        service.processOruR01(
            List.of(observation("ACC-1", "6.1", "1", "GLU", "N", "F")),
            hospital, "OTHER", "LAB_B", null, "MSH|...\r");

        ArgumentCaptor<LabResult> captor = ArgumentCaptor.forClass(LabResult.class);
        verify(labResultRepository).save(captor.capture());
        assertThat(captor.getValue().getId()).as("a fresh row, not a revision").isNull();
