 ...
 * unknown identifier does — one or two extra reads — so a patient
 * attacker with a stable socket could still separate the two
 ...
 *
 * <p>The reason a message was refused is not lost on the ORU^R01 path,
 * which records it on the {@code integration_message_event} row the
 * sender cannot read. The ADT and A40 paths write no such row yet — they
 * never have — so today their reason lives only in the application log.
 * Giving them one is a follow-up, deliberately not bundled here.
 *
