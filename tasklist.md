...
- [ ] 21. Registration extras: patient photo capture, consent-to-treat e-sign at check-in, guarantor accounts — 🔨 PR #474 open 2026-08-22 (`feature/registration-extras`, V126: photo columns + authenticated streaming endpoint kept OUT of the permitAll /uploads/** tree; consent-to-treat records incl. finally persisting the pre-check-in checkbox the backend used to discard; guarantor accounts; coverage-tab panels + check-in consent step + photo capture w/ webcam)
- [ ] 22. Recall lists + waitlist auto-offer (depends on #11)
- [ ] 23. Downtime/read-only continuity mode; wristband & label printing (scanning exists, printing doesn't) — 🔨 PR #475 open 2026-08-22 (`feature/downtime-printing`, STACKED on #474 — merge #474 first AND DELETE ITS BRANCH so GitHub retargets; V127: downtime singleton + HTTP write-blocking filter w/ X-Readonly-Mode discriminator + shell banner; wristband PDFs w/ bare-UUID QR (the eMAR five-rights contract) + specimen labels — first reader of barcode_value)
