...
- [ ] 49. **Replace the hospital-scoped read filter.** Today ~148 call sites
  ...
  ⚠ **Was blocked on encounter-write scoping; that block is cleared.** Ten
  mutating paths on `EncounterServiceImpl` (the six unscoped ones plus the
  three whose guard had a null-hospital bypass, plus `updateEncounter`, which
  could re-home a foreign encounter into the caller's tenant) now share one
  guard. What remains is that nothing *enforces* the invariant — see the
  entry below — so re-check it before widening reads rather than assuming it
  held.
