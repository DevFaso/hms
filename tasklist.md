...
1. **URGENT: `/users` account takeover** (`fix/users-endpoint-authorization`).
   ...
   See "`/users` has no role check on five of its seven endpoints" below. The missing username uniqueness
   check in `updateUser` is not in this PR; it is batch 2.
...
- `updateUser` has no username uniqueness check: a rename to a case variant
  of a live account leaves two rows the case-insensitive `findByUsername`
  cannot resolve, and breaks login for both (see the rename bullet near
  "`/users` has no role check on five of its seven endpoints").
...
- **Lab results are fetched cross-tenant and filtered in memory.** Owned by
  `fix/lab-result-patient-query-scope` (batch 1), which covers five callers,
  not the two named here, and keeps the ordered-OR-performed rule.
...
- **URGENT — an account takeover, live on prod. Owned by
  ...
  caller check, so the gap below is not a missing annotation but any
...
- **~~The FHIR read API was open to every authenticated user, across every
  ...
  **(cross-hospital)** still exposes something about another hospital:
  - **FHIR `Patient` read and search return 500 for every caller** (owned by
    `fix/fhir-patient-lazy-load`, batch 1) - on prod
...
- **~~Cross-tenant defects on the FHIR write and EMPI paths.~~ Closed by
  ...
  - Owned by `fix/empi-scope-and-event-timing` (batch 1): this item, the
    `/merge-by-patient` one and the Kafka one below.
  ...
  - `EmpiServiceImpl.publishEvent` sends to Kafka inside the transaction -
    ...
    change that never happened. Publish after commit, in one place.
