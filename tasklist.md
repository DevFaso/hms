...
- **Patient parity across web, Android and iOS (audit 2026-09-20).** The web portal is the reference. Missing on BOTH apps: the booking wizard (hospital → department → provider; both apps only offer doctors from past appointments, so a new patient cannot book), pre-check-in questionnaires, PRO self-screenings, patient education, medical/surgical/family/social history, the disclosures view + record-sharing opt-out (apps use the older access log), document upload/delete, forgot-password / activation / MFA challenge / change-password (both apps ship a dead Forgot-password button). Phase 1 (defects + app-to-app parity) = the `fix/*-patient-parity-phase1` PRs; phase 2 = the seven web features; phase 3 = the account flows; phase 4 = push notifications, chat attachments, Spanish. Phase 2 progress: the booking wizard on Android (`feat/android-patient-booking-wizard`, which also moves self-scheduling off the staff `POST /appointments` onto `/me/patient/appointments`); iOS booking, then the other six features, follow one PR each. Backend gap found on the way: `PatientPortalServiceImpl.scheduleMyAppointment` stores `notes` and drops `reason`, so the reason a patient types on the web or Android is lost (one-line fix, own PR off develop). Cross-cutting: most screens on both apps still swallow errors as empty states and carry hard-coded English.
