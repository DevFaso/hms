...
Eight per size, generated by `scripts/mobile-store-assets/generate.mjs` from
the e-Keneya mark and the apps' French string tables (login, dashboard,
appointments, lab results, medications, messages, vitals, billing — the
screens both apps have). Regenerate rather than edit a PNG.

| Display Size | Dimensions | Folder |
| ------------ | ---------- | ------ |
| iPhone 6.9" | 1320 × 2868px | `screenshots/6.9-inch/` |
| iPhone 6.7" | 1284 × 2778px | `screenshots/6.7-inch/` |
| iPhone 6.5" | 1242 × 2688px | `screenshots/6.5-inch/` |
| iPad 13" | 2048 × 2732px | `screenshots/ipad-13/` |

Upload the 6.9" set; App Store Connect derives the smaller iPhone slots from
it, or upload each set for full control. `project.yml` does not pin
`TARGETED_DEVICE_FAMILY`, and XcodeGen's default is iPhone + iPad, so the
iPad set is required unless the target is pinned to iPhone only.

The app icon is `MediHubPatient/Resources/Assets.xcassets/AppIcon.appiconset/AppIcon.png`
(1024 × 1024, opaque, from the same generator); App Store Connect reads it
from the build.
