...
All generated by `scripts/mobile-store-assets/generate.mjs` from the e-Keneya
mark (the woven cross in `hospital-portal/src/app/shared/brand-mark`), so the
store icon, the launcher icon and the web mark are one drawing. Regenerate
rather than edit a PNG.

| Asset | File | Size |
| --- | --- | --- |
| App icon | `store-assets/app-icon-512x512.png` | 512 × 512, opaque |
| Developer page icon | `store-assets/developer-icon-512x512.png` | 512 × 512 |
| Feature graphic | `store-assets/feature-graphic-1024x500.png` | 1024 × 500 |
| Phone screenshots (8) | `store-assets/phone/` | 1080 × 1920 |
| 7" tablet screenshots (8) | `store-assets/tablet-7inch/` | 1200 × 1920 |
| 10" tablet screenshots (8) | `store-assets/tablet-10inch/` | 1920 × 1200 |

Play requires the icon, the feature graphic and at least two phone
screenshots; the tablet sets are needed for the tablet listing badge only.
The launcher icon in `app/src/main/res` (adaptive vectors plus the API 23–25
rasters) comes from the same run.
