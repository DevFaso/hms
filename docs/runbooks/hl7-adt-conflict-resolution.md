...
| Cross-tenant     | sender ↔ hospital | The receiving hospital is the one allowlisted for the sender (MSH-3, MSH-4). The patient must already be `PatientHospitalRegistration`-bound to that hospital, otherwise the message is rejected with `REJECTED_NOT_FOUND` → **AE**, the same answer an MRN no hospital has ever heard of gets. Deliberate: an AR here and an AE there let an allowlisted sender walk an identifier space and learn which MRNs exist in hospitals it cannot read. **The ACK does not say "cross-tenant" and never will — look for the reason on the `integration_message_event` row** (`/super-admin/integration-messages`, SUPER_ADMIN), where the error message reads `cross-tenant rejection (MSH-10 ...)`. The row carries no message body on this path, by design. Note the row exists for the tenant and identifier refusals, **not** for the malformed-input ones (`REJECTED_INVALID`: PID-3 == MRG-1, a blank MRG-1, a merge the EMPI service itself refuses) — those leave a log line only. |
