...
| **Frontend UI** | `<hospital-typeahead>` standalone autocomplete used inside the chip and reusable elsewhere | ✅ shipped — [`hospital-typeahead.component.ts`](../hospital-portal/src/app/shared/hospital-typeahead/hospital-typeahead.component.ts) (first page on open, 300 ms debounce on keystrokes, "type to narrow" when the page is full, error/empty states, accessible listbox/option roles — #566) |
...
- `SuperAdminDashboardController.searchHospitalsForScopeChip` —
  ...
  belt-and-braces gating, first page on an empty query, server cap of 20,
